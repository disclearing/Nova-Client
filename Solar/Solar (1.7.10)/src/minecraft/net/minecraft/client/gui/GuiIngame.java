...
import com.orange.plump.Solar.modules.BossbarModule;
import com.orange.plump.Solar.modules.FancyPingModule;
import com.orange.plump.Solar.modules.PotionEffectHUDModule;
import com.orange.plump.Solar.modules.ScoreboardModule;
...
public class GuiIngame extends Gui
{
    ...
    private static final Font font = new Font("Century Gothic", Font.BOLD, 16);
    ...
    {
    	if (mc.cFontRenderer.getFont() != font) mc.cFontRenderer.setFont(font);
        ...
        if (PotionEffectHUDModule.toggled && Solar.doRender) {
			ScaledResolution var3 = new ScaledResolution(Minecraft.getMinecraft(), mc.displayWidth, mc.displayHeight);
            int var4 = var3.getScaledWidth();
            int var55 = var3.getScaledHeight();
			float mscale = (float)Math.pow(PotionEffectHUDModule.scale,-1);
			GL11.glScalef(PotionEffectHUDModule.scale, PotionEffectHUDModule.scale, PotionEffectHUDModule.scale);
				if (PotionEffectHUDModule.stayRight) {
					if (PotionEffectHUDModule.stayBottom) {
						PotionEffectHUDModule.drawHUD(var4 - PotionEffectHUDModule.x, var55 - PotionEffectHUDModule.y);
					} else {
						PotionEffectHUDModule.drawHUD(var4 - PotionEffectHUDModule.x, PotionEffectHUDModule.y);
					}
				} else {
					if (PotionEffectHUDModule.stayBottom) {
						PotionEffectHUDModule.drawHUD(PotionEffectHUDModule.x, var55 - PotionEffectHUDModule.y);
					} else {
						PotionEffectHUDModule.drawHUD(PotionEffectHUDModule.x, PotionEffectHUDModule.y);
					}
        ...
        if (!this.mc.playerController.enableEverythingIsScrewedUpMode())
        {
            ...
            Solar.onRenderGameOverlay(p_73830_1_);
            ...
            if (ArmorHUDModule.toggled && Solar.doRender) {
    			ScaledResolution var3 = new ScaledResolution(Minecraft.getMinecraft(), mc.displayWidth, mc.displayHeight);
                int var4 = var3.getScaledWidth();
                int var55 = var3.getScaledHeight();
                float mscale = (float)Math.pow(ArmorHUDModule.scale,-1);
                GL11.glScalef(ArmorHUDModule.scale, ArmorHUDModule.scale, ArmorHUDModule.scale);
    			if (ArmorHUDModule.stayRight) {
    				if (ArmorHUDModule.stayBottom) {
    					ArmorHUDModule.drawHUD(var4 - ArmorHUDModule.x, var55 - ArmorHUDModule.y);
    				} else {
    					ArmorHUDModule.drawHUD(var4 - ArmorHUDModule.x, ArmorHUDModule.y);
    				}
    			} else {
    				if (ArmorHUDModule.stayBottom) {
    					ArmorHUDModule.drawHUD(ArmorHUDModule.x, var55 - ArmorHUDModule.y);
    				} else {
    					ArmorHUDModule.drawHUD(ArmorHUDModule.x, ArmorHUDModule.y);
    				}
        ...
        if (this.mc.gameSettings.showDebugInfo)
        {
            ...
            GL11.glPushMatrix();
            var8.drawStringWithShadow("Solar 1.7.10 (" + this.mc.debug + ")", 2, 2, 16777215);
        ...
        if (this.mc.gameSettings.keyBindPlayerList.getIsKeyPressed() && (!this.mc.isIntegratedServerRunning() || this.mc.thePlayer.sendQueue.playerInfoList.size() > 1 || var37 != null))
        {
            ...
            for (var21 = 0; var21 < var15; ++var21)
            {
                ...
                if (var21 < var42.size())
                {
                    ...
                    if (FancyPingModule.isEnabled) {
                    	float size = 0.75F;
                        float msize = (float)Math.pow(size,-1);
                    	GL11.glScalef(size, size, size);
                    	int X = (int)((var22 + var46 - 9) / (size));
                    	int Y = (int)(var23 / (size))+ 2;
                        this.zLevel += 100.0F;
                        if (!isZero) mc.cFontRenderer.drawCenteredString(String.valueOf(var48.responseTime), X, Y, color);
                        this.zLevel -= 100.0F;
                        GL11.glScalef(msize, msize, msize);
                    } else {
                    	this.zLevel += 100.0F;
                    	this.drawTexturedModalRect(var22 + var46 - 12, var23, 0, 176 + var53 * 8, 10, 8);
                    	this.zLevel -= 100.0F;
                    }
                }
            }
        ...
        float msize = (float)Math.pow(size,-1);
        GL11.glScalef(size, size, size);
        int X = (int)((width/2) / size);
        int Y = (int)((height/2 - 8) / size);
        GL11.glEnable(GL11.GL_ALPHA_TEST);
        ...
        if (ScoreboardModule.isEnabled) {
            ...
            if (var6.size() <= 15)
            {
                ...
                for (Iterator var8 = var6.iterator(); var8.hasNext(); var7 = Math.max(var7, p_96136_4_.getStringWidth(var11)))
                {
                    ...
                    if (!ScoreboardModule.doScores) var11 = ScorePlayerTeam.formatPlayerName(var10, var9.getPlayerName());
                }
                ...
                while (var13.hasNext())
                {
                    ...
                    p_96136_4_.drawString(var16, var25, var19, 0xFFFFFF);
                    if (ScoreboardModule.doScores) p_96136_4_.drawString(var17, var20 - p_96136_4_.getStringWidth(var17), var19, 0xFFFFFF);
                    ...
                    if (var12 == var6.size())
                    {
                        ...
                        p_96136_4_.drawString(var21, var25 + var7 / 2 - p_96136_4_.getStringWidth(var21) / 2, var19 - p_96136_4_.FONT_HEIGHT, 0xFFFFFF);
                    }
                }
            }
        }
    ...
    {
        if (BossStatus.bossName != null && BossStatus.statusBarTime > 0 && BossbarModule.isEnabled)
        {
